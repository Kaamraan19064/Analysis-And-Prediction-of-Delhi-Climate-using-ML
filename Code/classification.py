...
def undersampling():
    ...
    df_0 = data[data['extreme_weather'].isin(y)]
    df_1=df_1[0:10000]
    df_0=df_0[0:10000]
    df=pd.concat([df_0,df_1])
    df = df.reindex(np.random.permutation(df.index))
    df_y=df.extreme_weather
    df = df.drop('extreme_weather', axis=1)
    print(df.shape)
    y=data.extreme_weather
    data = data.drop('extreme_weather', axis=1)
    # normalize(data)
    X_train, X_val, y_train, y_val = train_test_split(data, y, test_size=0.2, random_state=1)
    RandomForest_Extreme_weather_Feature_importance(df, X_val, df_y, y_val)

def Bar_graph(a1,a2,b1,b2,c1,c2,d1,d2,e1,e2):
    barWidth = 0.2
